...
export class FoodItemRepository extends BaseRepository<FoodItem> {
  ...
  async findWithoutAllergens(
    ...
    const conditions = allergens
      .map(() => `JSON_EXTRACT(allergens, '$') NOT LIKE ?`)
      .join(' AND ');
  ...
  async getBrands(): Promise<Array<{ brand: string; count: number }>> {
    ...
    const results = await this.executeQuery<{ brand: string; count: number }>(
      query
    );
    return results;
  }
  ...
  async getMostScanned(limit: number = 10): Promise<
    Array<{
      foodItem: FoodItem;
      scanCount: number;
    }>
  > {
    ...
    const results = await this.executeQuery<FoodItem & { scan_count: number }>(
      query,
      [limit]
    );
  ...
  // Get food trends
  async getFoodTrends(days: number = 30): Promise<
    Array<{
      foodName: string;
      brand: string;
      category: string;
