...
describe('FoodDatabaseService', () => {
  ...
  describe('analyzeFoodForGutHealth', () => {
    it('should analyze food for gut health conditions', async () => {
      ...
      const analysis = await service.analyzeFoodForGutHealth(foodItem, {
        conditions: {
          'lactose': { enabled: true, severity: 'moderate' as SeverityLevel, knownTriggers: [], lastUpdated: new Date() },
          'gluten': { enabled: false, severity: 'mild' as SeverityLevel, knownTriggers: [], lastUpdated: new Date() }
        },
        preferences: {
          dietaryRestrictions: [],
  ...
  describe('getSafeAlternatives', () => {
    it('should get safe alternatives for flagged food', async () => {
      ...
      const alternatives = await service.getSafeAlternatives(foodItem, {
        conditions: {
          'gluten': { enabled: true, severity: 'moderate' as SeverityLevel, knownTriggers: [], lastUpdated: new Date() }
        },
        preferences: {
          dietaryRestrictions: [],
          alternativePreferences: [],
  ...
  describe('getFoodRecommendations', () => {
    it('should get food recommendations based on profile', async () => {
      const profile = {
        conditions: {
          'ibs-fodmap': { enabled: true, severity: 'moderate' as SeverityLevel, knownTriggers: [], lastUpdated: new Date() }
        },
        preferences: {
          dietaryRestrictions: [],
          alternativePreferences: [],
